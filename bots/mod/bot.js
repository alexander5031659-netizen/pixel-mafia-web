...
const BOT_NAME = process.env.BOT_NAME || "Bot Mod v1.0";
const BOT_ID = process.env.BOT_ID || "mod-default";
...
console.log(`🛡️ ${BOT_NAME} [${BOT_ID}] iniciando...`);
...
(async () => {
    ...
    await sleep(2000);
    await enviar(`🛡️ ${BOT_NAME} activo - moderación en línea`);
    ...
    await page.exposeFunction("onMsg", ({ nombre, texto }) => {
        ...
        const ignorar = [botNameLower, "bienvenido", "llego", "hola a todos", "comandos", "error", "reglas"];
        if (!texto.startsWith("!")) {
